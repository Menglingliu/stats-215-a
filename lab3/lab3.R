...
Similarity <- function(Vector1, Vector2){      
  # binary_matrix1: q by q matrix with binary entries
  # binary_matrix2: q by q matrix with binary entries
  # return: the jaccard coefficient of binary matrix 1 and binary matrix 2
  
  # Construct the binary matrix based on the first clustering result for intersect index
  binary_matrix1 <- Vector1 %*% t(1/Vector1)
  binary_matrix1 <- (binary_matrix1==1)*1
  diag(binary_matrix1)<-0
  # Construct the binary matrix based on the second clustering result for intersect index
  binary_matrix2 <- (1/Vector2) %*% t(Vector2)
  binary_matrix2 <- (binary_matrix2==1)*1
  diag(binary_matrix2)<-0
  # Similarity function: Jaccard coefficient of binary matrix 1 and binary matrix 2
  similarity_matrix <- binary_matrix1 + binary_matrix2
  Jaccard <- sum(similarity_matrix==2) / (sum(similarity_matrix==1) + sum(similarity_matrix==2))
  return(Jaccard)
}
...
Similarity_output_R <- foreach (k = 2:10) %dopar% {
  for (i in 1:100){
    ...
    Similarity_score<-Similarity(cluster_index1,cluster_index2)
...
Duration_R <- Sys.time() - start.time
Duration_R
# Total running time is 23 mins

# Write the output into csv file
write.csv(Similarity_output_R,"Similarity_output_R.csv")
