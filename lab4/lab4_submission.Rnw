...
\title{Lab 4}
\author{Fan Dong (3033122445), Mengling Liu (3033122406), Nicholas Sim (26929549)}
...
Lastly, we create the density plots of all covariates by the expert label in Figure \ref{plot_density}, in order to check how the radiances and engineered features behave in the presence or absence of cloud. In line with the correlation plot, \texttt{NDAI} does the best job in separating cloudy region from the clear part, as the great majority of $\texttt{NDAI} > 1$ pixels indicate absence of cloud. The distributions of CORR and SD are not as indicative. While pixels with larger values across covariates are more likely to be labeled clear, in the lower ranges, the two labels mostly overlap, making it hard to separate one from another. The radiances of different angles share similar pattern. While the cloudy regions clearly have higher radiances, due to snow and ice-coverage, the radiance of clear regions spans the entire range. The large overlapping area makes it challenging to detect cloud coverage. \\
...
\begin{figure}[!htb]
  ...
  \endminipage
  \caption{Comparison of expert labels versus classifier predicted labels.}
  \label{visual}
\end{figure}

Our final selected classifier, \texttt{xgboost}, performed the best among all three classifiers in terms of accuracy. We provide the confusion matrix for \texttt{xgboost} in Table \ref{Confusion}.  The \texttt{xgboost} classifier also had a sensitivity value of 80.76\% and specificity value of  59.46\%. The Sensitivity-Specificity curve as well as the Precision-Recall curve are provided in Figure \ref{xgboostsensspec}.\\
\begin{table}[htbp]
\centering 
...
\begin{tabular}{l*{2}{cc}} 
\hline\hline
\textbf{} &\textbf{Predicted -1}  &\textbf{Predicted 1}\\
\hline
\texttt{Actual -1}     &$23006$         &$10746$\\
\hline
\texttt{Actual 1}      &$5481$          &$15763$\\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[!htb]
  \centering
  \minipage{0.5\textwidth}
    \includegraphics[width=\linewidth]{xgboostsensspec.pdf}
  \endminipage
  \minipage{0.5\textwidth}
    \includegraphics[width=\linewidth]{xgboostprerec.pdf}
  \endminipage
    \caption{Sens-spec curve and precision-recall curve for xgboosting}
  \label{xgboostsensspec}
\end{figure}

\newpage
\subsection{Comparison among all classifiers}
We want to delve a little deeper into the region in the bottom right corner ($x > 300$ and $y < 75$) of Image 3 where the expert label indicates that there were no clouds, whereas our classifiers tended to label almost all points in the region as cloudy.  To do so, we plotted the density curves for each covariate for that region in the holdout set (Image 3) in Figure \ref{plotdensityforsubset} together with the density curves for the learning set. It appears hard to obtain high accuracy for the data subset ($x > 300$ and $y < 75$) as covariate values in this region span across values in the learning set data with labels for both clear and cloudy data points.  In particular, if we look at the NDAI densities, we see that the mass of NDAI is centered around 1.7 in the test data subset and labeled as clear, whereas in our learning sets (Images 1 and 2), the density for clear points was centered much further to the left while the density for cloudy points was centered around 2.  This is indicates that our classfiers may not be sensitive enough to separate cloudy and clear regions in areas like this.  Especially for the portion with x-axis larger than 300, \texttt{lasso} and \texttt{xgboost} are doing a much better job than \texttt{ranger} but still fall short when $y<75$.  This leads us to question whether there are other potentially similar regions that could exist in other images and whether our classifier would perform similarly poorly there.\\

\begin{figure}[!htb]
  \minipage{\textwidth}
    \centering
    \includegraphics[width=0.8\linewidth]{plot_density_rc.pdf}
    \caption{Density plots of all covariates on data subset}
    \label{plotdensityforsubset}
  \endminipage
\end{figure}
